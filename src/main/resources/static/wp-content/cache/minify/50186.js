(function($){"use strict";$(document).ready(function(){$('.widget-babe-search-filter-terms').on('change','input:checkbox',function(el){if($('form#search_form').length>0){var term_taxonomy_id=$(this).val();if($(this).is(':checked')){$('form#search_form').append('<input type="hidden" name="terms['+term_taxonomy_id+']" value="'+term_taxonomy_id+'">');}else{$('form#search_form input[name="terms['+term_taxonomy_id+']"]').remove();}
babe_search_form_submit();}});$('input[name="sr_sort_by"]').on('change',function(el){if($('form#search_form').length>0){$('form#search_form input[name="search_results_sort_by"]').val($(this).val());babe_search_form_submit();}});$('.input_select_sort').on('click',function(el){if($('form#search_form').length>0){$('form#search_form input[name="search_results_sort_by"]').val($(this).children("input").val());babe_search_form_submit();}});$(document.body).bind('babe_price_slider_change',function(event,min,max){if($('form#search_form').length>0){if($('form#search_form input[name="min_price"]').length>0){$('form#search_form input[name="min_price"]').val(min);}else{$('form#search_form').append('<input type="hidden" name="min_price" value="'+min+'">');}
...
$('.add_input_field .add_ids_title').on('click',function(event){event.preventDefault();search_add_input_toggle(this);});$('.add_input_field .add_ids_list .term_item').on('click',function(event){event.preventDefault();event.stopPropagation();var parent=$(this).closest('.add_ids_title');search_add_input_toggle(parent);$(parent).find('input.input_select_input_value').attr('value',$(this).data('id'));$(parent).find('.term_item').removeClass('term_item_selected');$(this).addClass('term_item_selected');$(parent).find('.add_ids_title_value').html($(this).html());});function search_add_input_toggle(item){$(item).parent().find('.add_ids_list').toggleClass('active');$(item).parent().find('.add_ids_title i').toggleClass('fa-chevron-down');$(item).parent().find('.add_ids_title i').toggleClass('fa-chevron-up');}
function search_add_input_close(item){$(item).find('.add_ids_list').removeClass('active');$(item).find('.add_ids_title i').addClass('fa-chevron-down');$(item).find('.add_ids_title i').removeClass('fa-chevron-up');}
...
var par_guests=$(event.target).closest('.search_guests_field');if(!par_guests.length){$('#search_form .search_guests_select_wrapper').removeClass('is-active');}});function add_datepicker(id){$(id).datepicker({numberOfMonths:1,dateFormat:babe_lst.date_format});}
$(".search_date").on('focus',function(){$(this).blur();});$('#date_to.search_date').on('click',function(){$('#date_from.search_date').trigger('click.daterangepicker');});$('#date_from.search_date').daterangepicker({minDate:moment().format(babe_lst.drp_date_format),singleDatePicker:false,autoApply:true,autoUpdateInput:false,dateFormat:babe_lst.drp_date_format,customClass:'search-popup-date',widthSingle:500,timePickerIncrement:60,timePickerSeconds:false,timePicker:true,locale:{format:babe_lst.drp_date_format}});if($('#date_from.search_date').length>0){if(babe_lst.date_from!=null){$('#date_from.search_date').data('daterangepicker').setStartDate(babe_lst.date_from);}
if(babe_lst.date_to!=null){$('#date_from.search_date').data('daterangepicker').setEndDate(babe_lst.date_to);}}
$('#date_from.search_date').on('apply.daterangepicker',function(ev,picker){$(this).val(picker.startDate.format(babe_lst.drp_date_format));$('#date_to.search_date').val(picker.endDate.format(babe_lst.drp_date_format));});function is_stop_time(arr,hours){var count=0;var stop_time=false;var i;for(i in arr){if(arr.hasOwnProperty(i)){count++;var val=arr[i];var h=parseInt(i);if(h>=hours&&val==0){stop_time=true;}}}
return stop_time;}
...
return value;}
var av_cal_keys=$.map(babe_lst.av_cal,function(item,key){return key;});$(".booking_date").on('focus',function(){$(this).blur();});$('#booking_date_from').datepicker({numberOfMonths:1,dateFormat:babe_lst.date_format,maxDate:babe_lst.max_av_cal_date,beforeShowDay:function(date){var date_string=$.datepicker.formatDate('yy-mm-dd',date);var av_guests;var av_times_count;var check=false;if($.inArray(date_string,av_cal_keys)>-1){av_guests=get_obj_prop_max_value(babe_lst.av_cal[date_string]['times']);av_times_count=get_obj_prop_length(babe_lst.av_cal[date_string]['times']);if(av_guests>0&&babe_lst.av_cal[date_string]['start_day']>0){check=true;}}
return[check]},onSelect:function(dateText,inst){update_booking_time_select(true,inst);}});var date_to_before='';$('#booking_date_to').datepicker({numberOfMonths:1,dateFormat:babe_lst.date_format,maxDate:babe_lst.max_av_cal_date,beforeShow:function(){date_to_before='';},beforeShowDay:function(date){var date_string=$.datepicker.formatDate('yy-mm-dd',date);var av_guests=0;var prev_av_guests=0;var check=false;if($.inArray(date_to_before,av_cal_keys)>-1){prev_av_guests=get_obj_prop_max_value(babe_lst.av_cal[date_to_before]['times']);}
if($.inArray(date_string,av_cal_keys)>-1){av_guests=get_obj_prop_max_value(babe_lst.av_cal[date_string]['times']);if(av_guests>0||(babe_lst.basic_booking_period=='night'&&av_guests==0&&prev_av_guests>0)){check=true;}}
date_to_before=date_string;return[check]},onSelect:function(dateText,inst){update_booking_time_select(false,inst);}});var init_booking_calendars=true;if(babe_lst.date_from!=null){$('#booking_date_from').datepicker("setDate",babe_lst.date_from);$('.ui-datepicker-current-day').click();if(babe_lst.date_to!=null){setTimeout(function(){$('#booking_date_to').datepicker("setDate",babe_lst.date_to);$('.ui-datepicker-current-day').click();},700);}}
if(babe_lst.basic_booking_period=='single_custom'){get_times_guests();}
function update_date_to_datepicker(selectedYear,selectedMonth,selectedDay,date_from_updated){var selected_date=new Date(selectedYear,selectedMonth,selectedDay),start_date=new Date(selectedYear,selectedMonth,selectedDay),end_date=new Date(selectedYear+1,selectedMonth,selectedDay),$i=1;var selected_date_string=$.datepicker.formatDate('yy-mm-dd',selected_date);var time_from=$('#booking_time_from_block .input_select_field_booking_time_from .input_select_input').length>0?$('#booking_time_from_block .input_select_field_booking_time_from .input_select_input').val():'00:00';time_from=parseInt(time_from);if(isNaN(time_from)){time_from=0;}
...
function av_cal_second_click_update(Y1,m1,d1){var check=false;$('#av-cal').find('.cal-cell-active, .cal-cell-disabled-first').not('.cal-cell-disabled').each(function(el){var Y=parseInt($(this).data('year')),m=parseInt($(this).data('month'))-1,d=parseInt($(this).data('day'));if(!check&&$(this).hasClass('cal-cell-selected')){check=true;}
if(check&&(Y<Y1||(Y==Y1&&m<m1)||(Y==Y1&&m==m1&&d<=d1))){$(this).addClass('cal-cell-selected');}else{$(this).removeClass('cal-cell-selected');}});$('.cal-cell-active.cal-cell-disabled, .cal-cell-disabled-first.cal-cell-disabled').removeClass('cal-cell-disabled');$('.cal-cell-active.cal-cell-disabled-min').removeClass('cal-cell-disabled-min');cal_first_click=true;}
...
booking_form_calculate();function get_times_guests(){if($('.booking-guests-block').length>0){$('#booking_form_total').html('');var post_id=$('#booking_date_from').data('post-id'),date_from=$('#booking_date_from').val(),date_to='';if($('#booking_date_to').length>0){date_to=$('#booking_date_to').val();}
var guests=babe_lst.guests;$("#booking-guests-result .select_guests").each(function(){var age_id=$(this).data('age-id');guests[age_id]=$(this).val();});$('#booking-guests-result').html('<span class="spin_f"><i class="fas fa-spinner fa-spin fa-1x"></i></span>');$('#booking-times').html('<span class="spin_f"><i class="fas fa-spinner fa-spin fa-1x"></i></span>');$.ajax({url:babe_lst.ajax_url,type:'POST',data:{action:'get_times_guests',post_id:post_id,date_from:date_from,date_to:date_to,guests:guests,nonce:babe_lst.nonce},success:function(msg){$('#booking-times').html('');try{var response=JSON.parse(msg);}catch(e){return false;}
$('#booking-times').html(response.time_lines);$('#booking-guests-result').html(response.select_guests);booking_form_calculate();},error:function(msg){booking_form_calculate();}});}else{if($('.booking-time-block').length<=0){booking_form_calculate();}}}
$('#booking_form').on('change','input:checkbox',function(el){booking_form_calculate();});$('#booking_form').on('change','.input_select_field_booking_time_from .input_select_input_value',function(el){var date=$('#booking_date_from').datepicker("getDate");update_date_to_datepicker(date.getFullYear(),date.getMonth(),date.getDate(),false);booking_form_calculate();});$('#booking_form').on('change','.input_select_field_booking_time_to .input_select_input_value',function(el){booking_form_calculate();});$('#booking_form').on('change','input:radio[name=booking_time]:checked',function(el){var max_select_guests=parseInt($(this).data('max-select-guests'));var options='';$("#error_group").removeClass('show_error');for(i=0;i<=max_select_guests;i++){var add_class='';if(i==0){add_class=' term_item_selected';}
...
options+='<li class="term_item'+add_class+'" data-id="'+i+'" data-value="'+i+'">'+i+'</li>';}
$(select_item).val(selected_value);$(select_item_par).find('.input_select_list').html(options);av_guests=av_guests-selected_value;});booking_form_calculate();});$('#booking_form').on('change','input:radio[name=booking_meeting_point]:checked',function(el){var point_id=$(this).val();$("#error_group").removeClass('show_error');$('#block_meeting_points_default .meeting_point_default.meeting_point_default_selected').removeClass('meeting_point_default_selected');$('#block_meeting_points_default .meeting_point_default[data-point-id='+point_id+']').addClass('meeting_point_default_selected');$('#meeting_points_result .meeting_point.meeting_point_selected').removeClass('meeting_point_selected');$('#meeting_points_result .meeting_point[data-point-id='+point_id+']').addClass('meeting_point_selected');});function update_current_meeting_point(point_id){$('#booking_form input:radio[name=booking_meeting_point]:checked').prop('checked',false);$('#booking_form input#booking_meeting_point_'+point_id).prop('checked',true);$('#block_meeting_points_default .meeting_point_default.meeting_point_default_selected').removeClass('meeting_point_default_selected');$('#block_meeting_points_default .meeting_point_default[data-point-id='+point_id+']').addClass('meeting_point_default_selected');$('#meeting_points_result .meeting_point.meeting_point_selected').removeClass('meeting_point_selected');$('#meeting_points_result .meeting_point[data-point-id='+point_id+']').addClass('meeting_point_selected');}
function update_booking_time_select(first_time_select,inst){var sufx=first_time_select?'from':'to';var start_time=first_time_select?'00:00':$('#booking_time_from_block .input_select_field_booking_time_from .input_select_input').val();var time_selected=$('.booking-time-block input[name="booking_time_'+sufx+'"]').val();if(init_booking_calendars){time_selected=babe_lst['time_'+sufx];}
...
if(date_from!=''&&date_from!=undefined&&$('#booking_time_'+sufx+'_block').length>0){$('#booking_time_'+sufx+'_block').html('<span class="spin_f"><i class="fas fa-spinner fa-spin fa-1x"></i></span>');$.ajax({url:babe_lst.ajax_url,type:'POST',data:{action:'get_booking_times',post_id:post_id,date_from:date_from,time_selected:time_selected,start_time:start_time,from_to:sufx,nonce:babe_lst.nonce},success:function(msg){$('#booking_time_'+sufx+'_block').html(msg);},error:function(){$('#booking_time_'+sufx+'_block').html('');}}).always(function(){after_update_booking_time_select(first_time_select,inst);booking_form_calculate();});}else{after_update_booking_time_select(first_time_select,inst);}}
...
var guests={};$("#booking-guests-result .select_guests").each(function(){var age_id=$(this).data('age-id');guests[age_id]=$(this).val();});if(date_from!=''&&date_from!=undefined&&(!babe_lst.cal_first_click||(babe_lst.cal_first_click&&date_to!=''&&date_to!=undefined))){$('#booking_form_total').html('<span class="spin_f"><i class="fas fa-spinner fa-spin fa-2x"></i></span>');$.ajax({url:babe_lst.ajax_url,type:'POST',data:{action:'booking_calculate',post_id:post_id,data:$('#booking_form').serialize(),date_from:date_from,date_to:date_to,guests:guests,time_from:time_from,time_to:time_to,nonce:babe_lst.nonce},success:function(msg){$('#booking_form_total').html(msg);}});}}
$('#booking_form').on('click','.booking_form_submit',function(el){el.preventDefault();el.stopPropagation();$("#error_group").removeClass('show_error');var post_id=$('#booking_date_from').data('post-id'),date_from=$('#booking_date_from').val(),date_to='',check_meeting=1;if($('#booking_date_to').length>0){date_to=$('#booking_date_to').val();}
...
if(response.username==''){$('.new-username-check-msg').css('display','block');}
if(response.email==''){$('.new-email-check-msg').css('display','block');}
if(response.username!=''&&response.email!=''){$("#registration_form").submit();}},error:function(){$('#registration_form .form-spinner').css('display','none');$("#registration_form").submit();}});});if($('#user_registered_modal').length>0){$('#user_registered_modal').modal('show');}});function babe_initMap(){if($('#block_address_map').length>0){init_address_map('#google_map_address');}
if($('#block_meeting_points').length>0){init_meeting_map('#google_map_meeting_points','#block_meeting_points .address-autocomplete','#block_meeting_points .find_points');}
if($('#block_address_map_with_direction').length>0){init_address_map_with_direction('#google_map_address_with_direction','#block_address_map_with_direction .address-autocomplete','#travel_mode_panel');}}
